(() => {
  ...
  function bind(){
    ...
    $('#cp10FinishOnboarding')?.addEventListener('click',()=>{write(K.seen,true);$('#cp10Onboarding').close();announce('Bem-vindo à Central PDF versão 1.2.0')});
  ...
  function runCheck(){
    ...
    const checks=[
      ...
      {name:'Acessibilidade 1.2.0',detail:'Preferências e navegação por teclado ativas',ok:true}
    ];
  ...
  function enhanceHelp(){const d=$('#helpDialog .help-steps');if(d&&!$('#cp10HelpStable'))d.insertAdjacentHTML('afterend','<div id="cp10HelpStable" class="cp10-release-line"><div><strong>Atalhos da versão 1.2.0</strong><small>Ctrl K busca · Alt H início · Alt A acessibilidade · Alt Q qualidade · ? ajuda</small></div><b>Teclado</b></div>')}
  function init(){
